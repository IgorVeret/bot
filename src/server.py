...
def get_ngrok_url_init():
    ...
    for url_init in j['tunnels']: #Парсим ссылку ngrok
        if url_init['proto'] == 'https':
            res_url = url_init['public_url']
    return res_url

if __name__ == '__main__':
    logger.log(level=logging.INFO, msg='Запуск server')
    URL = get_ngrok_url_init()#Получаем ссылку
    print(URL)
    logger.log(level=logging.INFO, msg='Старт Ngrok Server ')
    my_bot.throw_webhook(f'{URL}/{TOKEN}')
    logger.log(level=logging.INFO, msg='Обратный вызов')
